...
function formatDocument(rawData) {
  ...
  data.forEach((issue, i) => {
    log(
      `[processing ${i + 1} of ${data.length}] ${issue.number}.${issue.title}`
    );
    const fm = [
      `layout: PostLayout`,
      `id: ${issue.number}`,
    ...
    const markdownText = `---\n${fm}\n---\n# ${issue.title}\n\n${issue.body}`;
    fs.writeFile(
      path.resolve(postPath, `./${issue.number}.md`),
      markdownText,
      () => {}
    );
  });
...
async function download() {
  ...
  try {
    let data = await tools.request("GET /repos/{owner}/{repo}/issues", {
      owner,
      repo,
    });
  ...
  try {
    const cates = await tools.request("GET /repos/{owner}/{repo}/milestones", {
      owner,
      repo,
    });
